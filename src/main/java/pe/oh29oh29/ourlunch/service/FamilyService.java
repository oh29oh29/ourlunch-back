...
public class FamilyService {
    ...
    public Family createFamily(RequestDTO.FamilyCreationRequest request) {
        ...
        FamilyMember familyMember = new FamilyMember(createdFamily, createdMember, request.getMemberName(), true, DateUtil.now());
    ...
    public void joinFamily(RequestDTO.FamilyJoinRequest request) {
        Member member = new Member(UUID.randomUUID().toString().toUpperCase(), "1234", DateUtil.now());
        Member createdMember = memberRepository.save(member);
